...
| 7 | EQ/EBB（含 ULT/音效） | 0x50-0x5B 全家族 | 预设 SET `[type][preset][bandCount][bands…]`；CUSTOM_EQ（0x31）SET `[type][bandCount][bands…]`（无 preset 字节，`gf0.g`/`hf0.a.c`）；ULT 型 SET `[type][basePreset][ultMode 0/1/2][bandCount][bands…]`；音效 SET `[0x30/0x33][SoundEffectType 0x00-0x06]`（整帧长 3）；Clear Bass `[type][level]` | 同构 + NTFY 失效宣告 | `gf0.C16901b` `hf0/c` `hf0/d` `gf0.z0/v0` |
| 8 | EQ 扩展信息 | 0x5A→0x5B | `[type]` | 频段/频点表；几何决定档位编码（SC `EqBandStepsStandard`/`EqBandSteps10band`）：SPECIFIC(Clear Bass)+5 频点 → raw 0..20 中心 10（±10）；10 频点（31 Hz…16 kHz）无 Clear Bass → raw 0..12 中心 6（±6） | `SonyEqEbbPayloadParser` |
