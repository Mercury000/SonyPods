...
import java.net.URL
import dev.sonypods.MainActivity
import dev.sonypods.SonyPodsApp
import dev.sonypods.PopupActivity
...
class SonyControlService : Service() {
    ...
    private fun bridgeState(uiState: SonyHeadphoneUiState) {
        ...
        if (address != null && lastConnectedAddress == null) {
            sendStateBroadcast(SonyPodsAction.ACTION_PODS_CONNECTED) {
        ...
        if (ancStatus != lastAncStatus) {
            ...
            sendStateBroadcast(SonyPodsAction.ACTION_PODS_ANC_CHANGED) {
                putExtra("address", address)
        ...
        if (ambientVoice != lastAmbientVoice) {
            ...
            sendStateBroadcast(SonyPodsAction.ACTION_PODS_AMBIENT_VOICE_CHANGED) {
                putExtra("address", address)
    ...
    private fun broadcastBattery(address: String, battery: BatteryParams, single: PodParams?) {
        ...
        sendStateBroadcast(SonyPodsAction.ACTION_PODS_BATTERY_CHANGED) {
            putExtra("address", address)
